...
public class MazeBug extends Bug {
	...
	private void findMaxCount(ArrayList<Location> locs){
		...
		for(int i = 0; i < locs.size(); i++){
			...
			if(dirCount[direction] > max){
				max = dirCount[direction];
				max_point = i;
			}
		}
		//只有一个位置可走
		if(locs.size() == 1){
			next = locs.get(0);
			//该方向次数加一
			dirCount[loc.getDirectionToward(locs.get(0))/90]++;
		}
		else{
			...
			if(num <= 6){
				...
				//该方向次数加一
				dirCount[loc.getDirectionToward(locs.get(max_point))/90]++;
			}
			//30%的可能性,默认走第一个
			else{
				next = locs.get(0);
